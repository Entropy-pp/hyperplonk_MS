    ...
    >,
{
    ...
    fn verify(
        ...
        // 少用 concat，多用 reserve
        let mut f_eval_point = f_rb.clone();
        f_eval_point.resize(f_num_vars, E::ScalarField::zero());
        let mut perm_eval_point = perm_rb.clone();
        perm_eval_point.resize(perm_num_vars, E::ScalarField::zero());
        ...
        for (poly, comms) in perm_f_hats.iter().zip(commitments.iter().skip(commitment_idx)) {
            for (mle, comm) in poly.flattened_ml_extensions.iter().zip(comms.iter()) {
                pcs_acc.insert_poly_and_points(mle, comm, &perm_eval_point);
                all_commitments.push(comm.clone());
                all_points.push(perm_eval_point.clone());
        ...
        // evaluations
        let evaluations: Vec<E::ScalarField> = batch_opening_proof.f_i_eval_at_point_i.clone();
        ...
        // f_folded_evals 验证
        let f_evals_match = f_hats.iter().enumerate().all(|(i, _)| {
            let expected = &f_folded_evals[i];
            (0..f_t).all(|j| {
                let computed = f_eq_rb_vec[i] * evaluations[eval_idx];
                eval_idx += 1;
                computed == expected[j]
            })
        });
        ...
        // perm_folded_evals 验证
        let perm_evals_match = perm_f_hats.iter().enumerate().all(|(i, _)| {
            let expected = &perm_folded_evals[i];
            (0..perm_t).all(|j| {
                let computed = perm_eq_rb_vec[i] * evaluations[eval_idx];
                eval_idx += 1;
                computed == expected[j]
            })
        });
        ...
        let duration = head.elapsed();
        println!("--------------666----------------{:?}",duration);
